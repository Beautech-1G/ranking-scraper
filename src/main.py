...
def split_maker_and_product(product_title: str, maker_from_page: str, list_maker_text: str) -> Tuple[str, str]:
        ...
        maker = page.evaluate(
            ...
            () => {
              ...
              function bad(t) {
                ...
                if (/Yahoo|ショッピング|ストア|ショップ|公式通販|店$/i.test(t)) return true;
                if (/選べるカラー/.test(t)) return true;
                if (/^[\\[【(].*[\\]】)]$/.test(t)) return true;
                return false;
              }
              ...
              for (let i = 0; i < 5 && node; i++) {
                ...
                if (
                  ...
                  Math.abs(r.left - h1Rect.left) <= 60 &&
                  r.width <= 260
                ) {
                  candidates.push(t);
                }
                ...
                for (const el of nested) {
                  ...
                  if (
                    !bad(tt) &&
                    rr.bottom <= h1Rect.top + 8 &&
                    Math.abs(rr.left - h1Rect.left) <= 60 &&
                    rr.width <= 260
                  ) {
                    candidates.push(tt);
                  }
...
def extract_detail_info(page: Page, candidate: RankingCandidate, mall: str) -> DetailInfo:
    ...
    maker_name, product_name = split_maker_and_product(
        product_title,
        maker_from_page,
